...
int get_managed_guest_bridge(char *brval, unsigned long length,unsigned int radio_index)
{
    bus_error_t rc;
    char *token = NULL;
    char *brname;
    ...
    rc = get_bus_descriptor()->bus_data_get_fn(&g_wifi_mgr->ctrl.handle, str,
        &data);
    if (data.data_type != bus_data_type_string) {
        wifi_util_error_print(WIFI_CTRL,
            "%s:%d '%s' SREESH bus_data_get_fn failed with data_type:0x%x, rc:%\n", __func__, __LINE__,
    ...
    if (rc == bus_error_success) {
        ...
        wifi_util_dbg_print(WIFI_CTRL, "SREESH Managed_wifi bridge name is %s\n", brname);
        token = strrchr(brname, ':');
        snprintf(brval, length, "%s", token + 1);
        wifi_util_info_print(WIFI_CTRL, "SREESH Managed_wifi bridge val is %s\n", brval);
        get_bus_descriptor()->bus_data_free_fn(&data);
        return RETURN_OK;
    }
